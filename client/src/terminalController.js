...
export default class TerminalController {
  ...
  #onLogChanged({ screen, activityLog }) {
    return (msg) => {
      const [userName] = msg.split(/\s/); // Jana left | Jana join
      const color = this.#getUserColor(userName);
  ...
  #onStatusChanged({ screen, status }) {
    return (users) => {
      const { content } = status.items.shift(); //get first element of list of users
      status.clearItems();
  ...
  async initializeTable(eventEmitter) {
    const components = new ComponentsBuilder()
      .setScreen({ title: "Terminal Hacker Chat - Janapc" })
      .setLayoutComponent()
